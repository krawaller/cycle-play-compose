...
function intent(
  state$: Stream<FormState>,
  confirmButtonClick$: Stream<any>
): Stream<string> {
...
function Form(sources: FormSources): FormSinks {
  const assignableInputSinks = isolate(
    AssignableInput,
    "fieldContent"
  )(sources) as FormSinks;
  ...
  const confirmButtonSinks = isolate(ConfirmButton, {
    state: confirmButtonLens,
  })(sources);

  const submittedName$ = intent(
    sources.state.stream,
    confirmButtonSinks.submit$
  );
